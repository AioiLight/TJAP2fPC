    ...
    //            CDTXMania.tテクスチャの解放( ref this.tx文字 );
		...
		public override int On進行描画()
		{
			if( !base.b活性化してない )
			{
                ...
                {
                    if (TJAPlayer3.Tx.SongSelect_ScoreWindow[TJAPlayer3.stage選曲.nNowSelectCource[0]] != null)
                    {
                        TJAPlayer3.Tx.SongSelect_ScoreWindow[TJAPlayer3.stage選曲.nNowSelectCource[0]].t2D描画(TJAPlayer3.app.Device, x, y);
                        this.t小文字表示(x + 56, y + 160, string.Format("{0,7:######0}", TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nハイスコア[TJAPlayer3.stage選曲.nNowSelectCource[0]].ToString()));
