...
namespace TJAPlayer3
{
	internal class CAct演奏演奏情報 : CActivity
	{
		...
		public void t進行描画( int x, int y )
		{
			...
			{
				y += 0x153;
				TJAPlayer3.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "BGM/Taiko Adj: {0:####0}/{1:####0} ms", TJAPlayer3.DTX[0].nBGMAdjust, TJAPlayer3.ConfigIni.nInputAdjustTimeMs ) );
				y -= 0x10;
				...
				y -= 0x10;
				TJAPlayer3.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "NoteE:         {0:####0}", TJAPlayer3.DTX[0].nノーツ数[1] ) );
				y -= 0x10;
				TJAPlayer3.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "NoteM:         {0:####0}", TJAPlayer3.DTX[0].nノーツ数[2] ) );
				y -= 0x10;
				TJAPlayer3.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "NoteC:         {0:####0}", TJAPlayer3.DTX[0].nノーツ数[3] ) );
				y -= 0x10;
				TJAPlayer3.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "SCROLL:        {0:####0.00}", this.dbSCROLL ) );
                y -= 0x10;
                TJAPlayer3.act文字コンソール.tPrint( x, y, C文字コンソール.Eフォント種別.白, string.Format( "SCOREMODE:     {0:####0}", TJAPlayer3.DTX[0].nScoreModeTmp ) );
                y -= 0x10;
